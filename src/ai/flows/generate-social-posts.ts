                 ...
                 if (error instanceof GenkitError) {
                     finalStatus = error.status ?? finalStatus;
                      ...
                      if (error.status === 503 || messageLower.includes('503') || messageLower.includes('overloaded') || messageLower.includes('service unavailable')) {
                          ...
                          finalMessage = `AI service was unavailable generating ${input.platform} post after multiple retry attempts.`; // Specific message
                      } else if (error.status === 'RESOURCE_EXHAUSTED' || messageLower.includes('rate limit exceeded')) {
                           finalStatus = 'RESOURCE_EXHAUSTED';
                           finalMessage = `AI service rate limit issues persisted generating ${input.platform} post after multiple retry attempts. Please check your Google API quota.`; // Specific message
                      } else {
