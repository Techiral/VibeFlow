...
import { Alert, AlertDescription, AlertTitle } from "@/components/ui/alert"; // Use AlertTitle here
...
export default async function LoginPage({
  ...
  const signIn = async (formData: FormData) => {
    ...
    try {
      ...
      if (serverActionError.message?.includes("NEXT_REDIRECT")) {
        throw serverActionError; // Re-throw redirect errors
      } else if (serverActionError.message?.includes('NEXT_')) {
  ...
  // Determine alert variant dynamically
  const isDestructiveMessage = (msg: string | null): boolean => {
    if (!msg) return false;
    const lowerMsg = msg.toLowerCase();
    const keywords = ["error", "could not", "limit reached", "configuration", "invalid", "failed"];
    return keywords.some(keyword => lowerMsg.includes(keyword));
  };
  const alertVariant = isDestructiveMessage(searchParams.message) ? "destructive" : "default";
        ...
        <CardContent>
          {searchParams?.message && (
            <Alert
              ...
              className={cn(
                "mb-4",
                alertVariant === "destructive" ? 'bg-destructive/10 border-destructive/30' : 'bg-primary/10 border-primary/30'
              )}
            >
              {!searchParams.message.toLowerCase().includes("check email") && (
                 <AlertCircle className="h-4 w-4" />
              )}
              <AlertTitle className="sr-only">{alertVariant === "destructive" ? "Error" : "Info"}</AlertTitle>
              <AlertDescription className={cn(
                    "text-center text-sm",
                    alertVariant === "destructive" ? 'text-destructive-foreground' : 'text-primary-foreground'
              )}>
                {searchParams.message}
              </AlertDescription>
            </Alert>
